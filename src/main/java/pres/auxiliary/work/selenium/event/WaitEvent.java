 ...
 * <p><b>用途：</b>
 * 定义了对元素进行特殊事件等待的方法，包括等待控件消失、等待控件出现文字等，
 * 可通过该类，等待元素进行一定的变化的操作
 * </p>
	...
	public boolean showText(Element element, String...keys) {
		...
		if (keys == null || keys.length == 0) {
			...
			if (isExistElement(element)) {
				...
				eventWait.withMessage("等待超时，元素“" + element.getElementData().getName() +"”仍未出现文本，超时时间：" + waitTime + "秒");
			...
			if (isExistElement(element)) {
				...
				eventWait.withMessage("等待超时，元素“" + element.getElementData().getName() +"”仍未出现关键词“" + keyText + "”，超时时间：" + waitTime + "秒");
