...
public abstract class EasyResponse {
    ...
    public InterfaceInfo getRequestInterfaceInfo() {
        return requestInterInfo.clone();
    }

    /**
     * 该方法用于对指定的内容根据传入的参数进行提取，返回需要提取到的内容，若未提取到内容，则返回内容本身
     * <p>
     * 提取规则如下：
     * <ol>
     ...
     * @param xpath         xml或html格式查找的xpath
     * @param leftBoundary  文本提取的左边界
     * @param rightBoundary 文本提取的右边界
     * @param index         边界提取到多条内容时指定的获取下标
     * @return 提取到的内容
     * @since autest 4.5.0
     */
    protected String extractKey(String text, Set<MessageType> bodyTypeSet, String paramName, String xpath,
            String leftBoundary, String rightBoundary, int index) {
        // 处理各个串参数，若其为null，则转换为空串
        paramName = Optional.ofNullable(paramName).orElse("");
        xpath = Optional.ofNullable(xpath).orElse("");
        text = Optional.ofNullable(text).orElse("");
        bodyTypeSet = Optional.ofNullable(bodyTypeSet).orElseGet(HashSet::new);

        // 根据参数，对传入的文本进行处理
        String value = analysisBody(text, paramName, xpath, bodyTypeSet);
     ...
     * @param bodyTypeSet 响应体文本格式集合
     * @return 查找到元素的内容
     * @since autest 4.5.0
     */
    protected String analysisBody(String bodyText, String paramName, String xpath, Set<MessageType> bodyTypeSet) {
     ...
     * @since autest 3.3.0
     */
    protected String disposeXmlParam(Document xml, String replaceSymbol, String[] paramNames, String xpath) {
     ...
     * @since autest 3.3.0
     */
    protected String disposeJsonParam(JSONObject json, String replaceSymbol, String[] paramNames) {
        // 对转换过程中的异常进行处理，若抛出异常，则直接返回空串
        try {
     ...
     * @since autest 3.3.0
     */
    protected Object getElement(String paramArrayName, String replaceSymbol, Object parentElement, short elementType,
     ...
     * @since autest 3.3.0
     */
    protected Entry<String, Integer> valueOfArrIndex(String name) {
     ...
     * @since autest 3.3.0
     */
    protected String disposeCompleteBoundaryText(String value, String leftBoundary, String rightBoundary, int index) {
     ...
     * @since autest 3.3.0
     */
    protected String disposeSingleBoundaryText(String value, String leftBoundary, String rightBoundary, int index) {
        // 判断非空边界，并获取非空边界内容
