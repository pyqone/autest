...
public class ExcelFileTemplet extends FileTemplet implements TempletAutoAddListSign<ExcelFileTemplet> {
    ...
    public ExcelFileTemplet setOrderedListSign(OrderedListSignType orderedListSignType, String... fields) {
        ...
        for (String field : fields) {
            ...
            // 记录当前需要使用的有序标记
            addFieldAttribute(field, KEY_SIGN_TYPE, orderedListSignType.getCode());
        }
    ...
    @Override
    public ExcelFileTemplet setUnorderedList(String signText, String... fields) {
        // 若传入的枚举为null，则不进行写入
        if (!Optional.ofNullable(signText).filter(text -> !text.isEmpty()).isPresent()) {
            return this;
        }
