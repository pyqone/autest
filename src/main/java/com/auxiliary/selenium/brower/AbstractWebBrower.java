 ...
 * </p>
 * <p>
 * <b>用途：</b>该类定义启动浏览器时必要的方法，开启浏览器的方法由子类继承编写
 * </p>
 * <p>
...
public abstract class AbstractWebBrower extends AbstractBrower {
	 ...
	 */
	public AbstractWebBrower(File driverFile, String url, String pageName) {
		this(driverFile);
	 ...
	 */
	public AbstractWebBrower(File driverFile, Page page) {
		this(driverFile);
		nowPage = page;
	}
	...
	private String getPopuHandle(Set<String> handleSet) {
		return handleSet.stream().filter(handle -> findPage(handle) == null).findAny()
				.orElseThrow(() -> new IncorrectPageException("不存在允许访问的标签"));
	}
}
