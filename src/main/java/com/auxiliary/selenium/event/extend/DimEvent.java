...
/**
 * <p><b>文件名：</b>DimEvent.java</p>
 * <p><b>用途：</b>
 * 简化事件写法的模糊事件，根据元素的类型和默认值，自动关联元素的点击和输入事件，以简化事件的编写。
 * </p>
 * <p>
 * 在类中，定义了获取元素文本内容的关键词“get”和清空文本的关键词“clear”，当元素的默认值为该字符串时，
 * 则将方法定义为获取元素的文本内容，故在编写元素默认值时，需要注意该关键词，避免造成不符合预期的结果。
 * </p>
 * <p><b>编码时间：</b>2021年3月23日下午6:29:22</p>
 * <p><b>修改时间：</b>2021年3月23日下午6:29:22</p>
 * @author 彭宇琦
 * @version Ver1.0
 * @since JDK 1.8
 */
public class DimEvent extends AbstractEvent {
	 ...
	 * <p>
	 * 方法支持传入正则表达式，当默认值满足传入的正则时，则会将其按照在公式中定义
	 * 的处理方式对数据进行处理，生成对应的数据。例如：
	 * <code><pre>
	 ...
	 * @return 事件的返回值集合
	 */
	public List<String> event(Element...elements) {
						...
						while((index = defaultValue.indexOf(key)) > -1) {
							if (index == 0) {
								...
								defaultValue = defaultValue.substring(0, index) 
										+ repleatMap.get(key) + defaultValue.substring(index + key.length());
							}
						}
					}
